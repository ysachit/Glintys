...
if($_POST)
{
	...
	if(isset($_POST["message"]) &&  strlen($_POST["message"])>0)
	{
		...
		if(mysql_query($sql_con,"INSERT INTO shout_box(user, message, ip_address) value('$username','$message','$user_ip')"))
		{
	...
	{
		$results = mysql_query($sql_con,"SELECT user, message, date_time FROM (select * from shout_box ORDER BY id DESC LIMIT 10) shout_box ORDER BY shout_box.id ASC");

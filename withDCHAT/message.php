...
<?php
					...
					if($yr=="$userid")
					{
						...
						$dwdm=mysql_fetch_array($sda);
							...
							echo '<a href=message.php?id=' . $dwdm["user_id"] . '>'.$dwdm['fname']." ".$dwdm['lname'];
							echo '</td>';
							echo '</tr>';									
					}
					else
					{
					    ...
					    $dw=mysql_fetch_array($hit);
							echo '<tr>';
							echo '<td class="namehere">';
							echo '<a href=message.php?id=' . $dw["user_id"] . '>'.$dw['fname']." ".$dw['lname'];
							echo '</td>';
							echo '</tr>';
...
if (isset($_GET['id']))
    ...
    echo '<input name="chat_message" id="chat_message" type="text" placeholder="Type Message Hit Enter" maxlength="500" /> ';
    echo '</div>';
echo '</div>';
				}
		?> 
